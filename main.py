...
def render_stock_detail(tkr):
    ...
    <div class="hero">
      ...
      <div style="margin-top:6px;font-size:0.85rem;color:var(--muted);">
        NSE&nbsp;&nbsp;<span style="font-size:1.1rem;font-weight:700;color:var(--text);">{price_str}</span>&nbsp;&nbsp;{chg_line}
      </div>
    </div>""", unsafe_allow_html=True)
        ...
        with row_box:
            ...
            with rc2:
                st.markdown(f"""<div style="text-align:right;padding-top:6px;">
                  <div class="row-val">{price_html}</div><div class="row-val-sub">{chg_html}</div>
                </div>""", unsafe_allow_html=True)
