...
.up{color:var(--green);}
.down{color:var(--red);}
.big-buy button{background:var(--blue)!important;border-color:var(--blue)!important;color:#fff!important;font-size:1rem!important;padding:14px 0!important;font-weight:700!important;}
.big-sell button{background:var(--red)!important;border-color:var(--red)!important;color:#fff!important;font-size:1rem!important;padding:14px 0!important;font-weight:700!important;}
...
.stock-name-btn button{background:transparent!important;border:none!important;padding:0!important;text-align:left!important;font-size:1rem!important;font-weight:700!important;color:var(--text)!important;justify-content:flex-start!important;}
...
.st-key-wl_remove div[data-testid="stButton"] button{background:transparent!important;border:none!important;color:var(--muted)!important;font-size:0.95rem!important;padding:0!important;box-shadow:none!important;}
    ...
    for tkr in active_list:
        ...
        with rc1:
            ...
            if st.button(tkr.replace('.NS', ''), key=f"open_{tkr}", use_container_width=True):
                ...
                st.rerun()
            st.markdown(f'<div style="font-size:0.7rem;color:var(--muted);margin-top:-6px;">NSE{hold_html}</div>',
                        unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with rc2:
