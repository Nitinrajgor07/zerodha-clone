...
def render_watchlist():
    ...
    prices = fetch_prices(tuple(st.session_state.watchlist))
    rows = ['<div class="row-card">']
    ...
    with st.expander("Remove a stock"):
        rem = st.selectbox("Select", st.session_state.watchlist, label_visibility="collapsed")
        if st.button("Remove"):
...
def render_orders():
    ...
    with st.form("order_form", clear_on_submit=True):
        ...
        if submitted and tkr.strip():
            ...
            price = p.get("price")
            if price is None:
                st.error("Price nahi mila — ticker check karo.")
            else:
                cost = price * qty
                h = st.session_state.holdings
                if side == "BUY":
                    if cost > st.session_state.cash:
                        st.error("Cash kam hai.")
                    else:
                        ...
                        st.session_state.orders.insert(0, {"ticker": t, "side": "BUY", "qty": qty, "price": price,
                                                            "time": datetime.datetime.now(IST).strftime("%d %b, %I:%M %p")})
                        st.success(f"Bought {qty} {t.replace('.NS','')} @ ₹{price:,.2f}")
                else:
                    if t not in h or h[t]["shares"] < qty:
                        st.error("Itni quantity holding mein nahi hai.")
                    else:
                        h[t]["shares"] -= qty
                        if h[t]["shares"] == 0:
                            del h[t]
                        st.session_state.cash += cost
                        st.session_state.orders.insert(0, {"ticker": t, "side": "SELL", "qty": qty, "price": price,
                                                            "time": datetime.datetime.now(IST).strftime("%d %b, %I:%M %p")})
                        st.success(f"Sold {qty} {t.replace('.NS','')} @ ₹{price:,.2f}")
...
PAGES = {"home": ("Home", render_home), "watchlist": ("Watchlist", render_watchlist),
         "portfolio": ("Portfolio", render_portfolio), "orders": ("Orders", render_orders)}
